package mods.vivaanmc.optimium.common.config;
...
/**
 * Documentation of these options (For optimium, not 100% accurate for Optimium): https://github.com/jellysquid3/sodium-fabric/wiki/Configuration-File
 */
@SuppressWarnings("CanBeFinal")
public class optimiumConfig {
    private static final Logger LOGGER = LogManager.getLogger("optimiumConfig");

    private static final String JSON_KEY_optimium_OPTIONS = "optimium:options";

    private final Map<String, Option> options = new HashMap<>();

    private optimiumConfig() {
        ...
        this.addMixinRule("features.shader", false);
        this.addMixinRule("features.removeGrassColor", false);
    }
    ...
    private void applyModOverrides() {
        for (ModContainer container : FabricLoader.getInstance().getAllMods()) {
            ...
            if (meta.containsCustomValue(JSON_KEY_optimium_OPTIONS)) {
                CustomValue overrides = meta.getCustomValue(JSON_KEY_optimium_OPTIONS);

                if (overrides.getType() != CustomValue.CvType.OBJECT) {
                    LOGGER.warn("Mod '{}' contains invalid optimium option overrides, ignoring", meta.getId());
                    continue;
                }
     ...
     */
    public static optimiumConfig load(File file) {
        if (!file.exists()) {
            try {
            ...
            optimiumConfig config = new optimiumConfig();
    ...
    private static void writeDefaultConfig(File file) throws IOException {
        ...
        try (Writer writer = new FileWriter(file)) {
            writer.write("# This is the configuration file for optimium.\n");
            writer.write("#\n");
            writer.write("# You can find information on editing this file and all the available options here:\n");
            writer.write("# https://github.com/jellysquid3/optimium-fabric/wiki/Configuration-File\n");
            writer.write("#\n");
