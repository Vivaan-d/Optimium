...
import mods.vivaanmc.optimium.client.gui.options.control.ControlElement;
import mods.vivaanmc.optimium.client.gui.options.storage.OptionStorage;
import mods.vivaanmc.optimium.client.gui.widgets.FlatButtonWidget;
...
public class OptimiumOptionsGUI extends Screen {
    ...
    public OptimiumOptionsGUI(Screen prevScreen) {
        super(Text.translatable("optimium Options"));

        this.prevScreen = prevScreen;

        this.pages.add(optimiumGameOptionPages.general());
        this.pages.add(optimiumGameOptionPages.quality());
        this.pages.add(optimiumGameOptionPages.performance());
        this.pages.add(optimiumGameOptionPages.advanced());
    }
    ...
    private void rebuildGUI() {
        ...
        this.undoButton = new FlatButtonWidget(new Dim2i(this.width - 211, this.height - 30, 65, 20), Text.translatable("optimium.options.buttons.undo"), this::undoChanges);
        this.applyButton = new FlatButtonWidget(new Dim2i(this.width - 142, this.height - 30, 65, 20), Text.translatable("optimium.options.buttons.apply"), this::applyChanges);
        this.closeButton = new FlatButtonWidget(new Dim2i(this.width - 73, this.height - 30, 65, 20), Text.translatable("gui.done"), this::close);
        this.donateButton = new FlatButtonWidget(new Dim2i(this.width - 128, 6, 100, 20), Text.translatable("optimium.options.buttons.donate"), this::openDonationPage);
        this.hideDonateButton = new FlatButtonWidget(new Dim2i(this.width - 26, 6, 20, 20), Text.literal("x"), this::hideDonationButton);

        if (optimiumClientMod.options().notifications.hideDonationButton) {
    ...
    private void hideDonationButton() {
        optimiumGameOptions options = optimiumClientMod.options();
    ...
    private void renderOptionTooltip(MatrixStack matrixStack, ControlElement<?> element) {
        ...
        if (impact != null) {
            tooltip.add(Language.getInstance().reorder(Text.translatable("optimium.options.performance_impact_string", impact.getLocalizedName()).formatted(Formatting.GRAY)));
        }
